...
#+begin_src emacs-lisp
  (leaf tabspaces
    :straight t
    :hook (after-init-hook . tabspaces-mode) ;; use this only if you want the minor-mode loaded at startup.
...
動作が怪しい（カーソルを合わせても反応しない・フレームが消えない）
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (leaf sideline
    ...
    :init
    (leaf sideline-flycheck :straight t :hook (flycheck-mode-hook . sideline-flycheck-setup))
    (leaf sideline-flymake :straight t)
    (leaf sideline-lsp :straight t)
...
#+begin_src emacs-lisp
  (leaf cargo-compete
    ...
    :init
    (defvar cargo-compete-atcoder-submit-mode 2) ;; use crate but no binary sujbmit
